...
export const create: <Payload>(deps: Dependencies) => (props: Props<Payload>) => API.Authorizer<Payload> = <Payload>() => (props) => {
	...
	const tokenDefaultClaimsSchema = z.object({
		...
		sub: z.string(),
		aud: z.literal(audience).or(z.array(z.string()).refine((val) => val.includes(audience))),
	});
	...
	return {
		authorize: async ({ token }) => {
			...
			try {
				...
				const parsedPayload = payloadSchema?.parse(payload) as unknown as Payload;
