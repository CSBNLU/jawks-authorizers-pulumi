...
export const create: <AccessTokenPayload, RefreshTokenPayload>(deps: Dependencies<AccessTokenPayload, RefreshTokenPayload>) => (props: Props) => AuthorizerArgs = (deps) => (props) => ({
	...
	handler: new aws.lambda.CallbackFunction(`${props.prefix}-refresh-token-authorizer`, {
		architectures: ["arm64"],
		callback: lambdaAuthorizer.create({ authorizer: deps.jwtModule.refreshTokenAuthorizer }),
	}),
});
